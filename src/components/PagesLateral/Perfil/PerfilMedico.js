...
{perfil.pliegues && (
  <>
    <h4 className="fxh-subsection">Pliegues cutáneos (mm)</h4>

    {mostrar("Pectoral:", perfil.pliegues?.pectoral)}
    {mostrar("Abdominal:", perfil.pliegues?.abdominal)}
    {mostrar("Tricipital:", perfil.pliegues?.tricipital)}
    {mostrar("Subescapular:", perfil.pliegues?.subescapular)}
    {mostrar("Suprailiaco:", perfil.pliegues?.suprailiaco)}
    {mostrar("Muslo:", perfil.pliegues?.muslo)}
  </>
)}
          ...
          {/* ================= EXÁMENES ================= */}
          <h3 className="fxh-section">Tus exámenes</h3>
          
          {mostrar("Glicemia:", perfil.glicemia)}
          {mostrar("Hemoglobina glicosilada (HbA1c):", perfil.hba1c)}


          {/* --------- SANGRE --------- */}
          
          <h4 className="fxh-subsection">Exámenes de sangre</h4>
          
          {mostrar("Hemoglobina:", perfil.hemoglobina)}
          {mostrar("Hematocrito:", perfil.hematocrito)}
          {mostrar("Leucocitos:", perfil.leucocitos)}
          {mostrar("Plaquetas:", perfil.plaquetas)}
          
          {mostrar("Colesterol total:", perfil.colesterolTotal)}
          {mostrar("HDL:", perfil.hdl)}
          {mostrar("LDL:", perfil.ldl)}
          {mostrar("Triglicéridos:", perfil.trigliceridos)}
          
          {mostrar("AST (hígado):", perfil.ast)}
          {mostrar("ALT (hígado):", perfil.alt)}
          {mostrar("Bilirrubina:", perfil.bilirrubina)}
          
          {mostrar("Creatinina:", perfil.creatinina)}
          {mostrar("Urea:", perfil.urea)}
          
          {mostrar("Sodio:", perfil.sodio)}
          {mostrar("Potasio:", perfil.potasio)}


          {/* --------- ORINA --------- */}
          
          <h4 className="fxh-subsection">Exámenes de orina</h4>
          
          {/* Características */}
          {mostrar("Color:", perfil.colorOrina)}
          {mostrar("Aspecto:", perfil.aspectoOrina)}
          {mostrar("Densidad:", perfil.densidadOrina)}
          {mostrar("pH:", perfil.phOrina)}
          
          
          {/* Químico */}
          {mostrar("Proteínas:", perfil.proteinasOrina)}
          {mostrar("Glucosa:", perfil.glucosaOrina)}
          {mostrar("Cetonas:", perfil.cetonasOrina)}
          
          {/* Microscópico */}
          {mostrar("Leucocitos:", perfil.leucocitosOrina)}
          {mostrar("Eritrocitos:", perfil.eritrocitosOrina)}
          {mostrar("Bacterias:", perfil.bacteriasOrina)}

...
const HistorialTrote = () => {
  ...
  useEffect(() => {
    const obtenerHistorial = async () => {
      ...
      try {
        const res = await axios.get(`http://10.93.154.208:4000/api/trote/historial/${usuario_id}`);
  ...
  return (
    <div style={{ maxWidth: '60rem', margin: '2rem auto', padding: '2rem', backgroundColor: '#f9f9f9', borderRadius: '12px' }}>
      <h3 style={{ textAlign: 'center', marginBottom: '1rem' }}>📈 Gráfico de calorías quemadas trotando</h3>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={historial.map((item, i) => ({ ...item, index: i + 1 }))}>
          ...
          <Tooltip />
          <Line type="monotone" dataKey="calorias" stroke="#03a9f4" strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>

      <h3 style={{ marginTop: '2rem' }}>📋 Historial de sesiones de trote</h3>

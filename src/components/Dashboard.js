...
useEffect(() => {
  ...
  const fetchEmocion = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/emociones/${usuario_id}`
      );
      ...
      if (!cancel) {
        ...
        const ultima = data.length
          ? data.reduce((acc, cur) =>
              new Date(cur.fecha) > new Date(acc.fecha) ? cur : acc
            )
          : null;
      ...
      console.error("Error emociones:", error.response?.data || error.message);
    }
  };
...
console.log("EMOCION DASHBOARD:", emocion);
...
useEffect(() => {
  const fetchSueno = async () => {
    try {
      ...
      console.log("DATA SUENO:", res.data);
...
if (res.data.length > 0) {
  ...
  setUltimoSueno(ordenado[0]);
  console.log("SET SUENO:", ordenado[0]);
}
    } catch (error) {
...
// Calcula la cantidad de horas dormidas entre horaDormir y horaDespertar
function calcularHorasDormidas(horaDormir, horaDespertar) {
  ...
  // Si la hora de despertar es menor que la de dormir, asumimos que pasó a la mañana siguiente
  if (fin <= inicio) fin += 24;

  return Math.round((fin - inicio) * 10) / 10; // redondea a 1 decimal
}

useEffect(() => {
  console.log("SUEÑO EN ESTADO:", sueno);
}, [sueno]);
      ...
      const totales = registros.reduce(
        (acc, comida) => {
          ...
          alimentos.forEach((al) => {
            const qty = Number(al.cantidad) || 1;
            acc.proteinas     += (Number(al.proteinas)     || 0);
            acc.grasas        += (Number(al.grasas)        || 0);
            acc.carbohidratos += (Number(al.carbohidratos) || 0);
            acc.fibra         += (Number(al.fibra)         || 0);
          });
    ...
    <>
      ...
      <Typography className="parrafo31">
        <strong>💨Frecuencia respiratoria:</strong> {signosVitales.frecuencia_respiratoria} rpm
          ...
          {(yoga || emocion) && (  // 🔥 Solo si hay un registro de yoga aparece el módulo completo
...
>
  ...
  <Box sx={{ marginTop: 3 }}>
    ...
    <Typography sx={{ fontSize: "1.3rem", textAlign: "center" }}>
      💭 <strong>Emoción:</strong> {emocion.emocion}
    </Typography>

    <Typography sx={{ fontSize: "1.3rem", textAlign: "center" }}>
      📝 <strong>Sentimiento:</strong> {emocion.sentimiento}
    </Typography>

    <Typography sx={{ fontSize: "1.3rem", textAlign: "center" }}>
      📄 <strong>Descripción:</strong> {emocion.descripcion}
    </Typography>

    <Typography sx={{ fontSize: "1.3rem", textAlign: "center" }}>
      📅 <strong>Fecha:</strong> {emocion.fecha}
    </Typography>
  </Box>
)}
  ...
  <MenuItem onClick={cerrarMenuSueno}>
    ...
    {sueno ? (
      <>
        ...
        <Typography className="parrafo3" sx={{ fontSize: "1.7rem" }}>
          <strong>Comentarios:</strong> {sueno.notas || "Ninguno"}
        </Typography>
...
>
  ...
  <MenuItem onClick={cerrarMenuFarmacos}>
    ...
    <Typography className="parrafo2" sx={{ fontSize: '2.4rem', textAlign: 'center', fontWeight: 650, mb: '3rem' }}>
        <Link to="/farmacos/registrofarmacos/registrofarmacoscalendario" > 📅 Registro Medicamentos   </Link>
    </Typography>
...
>
  ...
  <MenuItem onClick={cerrarMenuFarmacos}>
    ...
    <Typography className="parrafo2" sx={{ fontSize: '2.4rem', fontWeight: 650, mb: '2rem', textAlign: 'center' }}>   <Link to="/skincarepresentacion/skincaredashboard" > 🧴 Cuidado de la piel    </Link>  </Typography>

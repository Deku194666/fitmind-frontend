...
import { FaPrayingHands, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroYoga2.css";

// Card de métricas con icono
...
function RegistroYoga2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/yoga/historial/${usuario_id}`,
        ...
        console.error("Error al cargar yoga:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "yoga") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo21">🧘 Análisis de Yoga</p>

      <div className="intro">
        Visualiza la evolución de tus sesiones de yoga: duración total,
        calorías quemadas e intensidad promedio. Ideal para mejorar
        flexibilidad, equilibrio y concentración.
      </div>

      {/* Resumen de métricas */}
      <div className="resumen21">
        <MetricCard icon={<FaPrayingHands size={28} color="#4f46e5" />}>

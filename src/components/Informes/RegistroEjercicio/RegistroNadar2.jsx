...
import { FaSwimmer, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroNadar2.css";
...
function RegistroNadar2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/nadar/historial/${usuario_id}`,
        ...
        console.error("Error al cargar nadar:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "nadar") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo">🏊‍♂️ Análisis de Natación</p>

      <div className="intro">
        Analiza tus sesiones de natación: duración total, calorías quemadas
        e intensidad promedio. Ideal para mejorar resistencia y salud cardiovascular.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaSwimmer size={28} color="#0ea5e9" />}>

...
function RegistroBoxeoSaco2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        // ⛳️ Endpoint backend esperado: GET /api/boxeosaco/historial/:usuario_id
        ...
        console.error("Error al cargar boxeosaco:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    // 🔁 Auto-refresco al registrar desde el módulo de boxeo en saco
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "boxeosaco") fetchRegistros();
    };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <p className="Par1">📊 Análisis de Boxeo en Saco</p>
      <p className="Par2">Visualiza la evolución de tus sesiones de boxeo en saco.</p>

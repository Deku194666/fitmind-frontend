...
import { FaDumbbell, FaClock, FaFire, FaStar } from "react-icons/fa";
...
function RegistroMusculacion2() {
  ...
  useEffect(()=>{
    ...
    const onNuevo = e=>{ if(!e?.detail?.tipo || e.detail.tipo==="musculacion") fetchRegistros(); };
  ...
  const promedioIntensidad = data.length>0
    ?(data.reduce((acc,x)=>acc+(x.kcal_x_min||0),0)/data.length).toFixed(2):0;
  const mejorSesion = data.reduce((max,x)=>x.calorias>(max.calorias||0)?x:max,{});
  const chartWidth = Math.max(data.length*90,600);

  return (
    <div className="registro-container">
      <p className="titulo22">💪 Análisis de Musculación</p>

      <div className="intro">
        Analiza tus sesiones de musculación: duración, calorías y progreso.
      </div>

...
import { FaBicycle, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroBicicleta2.css";
...
function RegistroBicicleta2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/bicicleta/historial/${usuario_id}`,
        ...
        console.error("Error al cargar bicicleta:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "bicicleta") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo">🚴 Análisis de Bicicleta</p>

      <div className="intro">
        Analiza tus sesiones de ciclismo: duración total, calorías quemadas
        e intensidad promedio. Ideal para mejorar resistencia y rendimiento.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaBicycle size={28} color="#0ea5e9" />}>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="minutos" stroke="#0ea5e9" dot={{ r: 3 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
...
export default RegistroBicicleta2;

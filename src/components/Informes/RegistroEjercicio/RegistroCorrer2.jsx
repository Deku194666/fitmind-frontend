...
function RegistroCorrer2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/correr/historial/${usuario_id}`,
        ...
        console.error("Error al cargar correr:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "correr") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo">🏃‍♂️ Análisis de Correr</p>

      <div className="intro">
        Aquí puedes analizar tus sesiones de carrera: duración total, calorías
        quemadas e intensidad promedio. Visualiza tu progreso y detecta tu mejor rendimiento.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaRunning size={28} color="#f97316" />}>
...
export default RegistroCorrer2;

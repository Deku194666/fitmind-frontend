...
import { FaFistRaised, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroBoxeoSaco2.css";
...
function RegistroBoxeoSaco2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/boxeosaco/historial/${usuario_id}`,
        ...
        console.error("Error al cargar boxeo de saco:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "boxeo-saco") fetchRegistros();
    };
  ...
  const promedioIntensidad = data.length > 0
    ? (data.reduce((acc, x) => acc + (x.kcal_x_min || 0), 0) / data.length).toFixed(2)
    : 0;
  const mejorSesion = data.reduce((max, x) => (x.calorias > (max.calorias || 0) ? x : max), {});

  return (
    <div className="registro-container">
      <p className="titulo">🥊 Análisis de Boxeo de Saco</p>

      <div className="intro">
        Analiza tus sesiones de boxeo de saco: duración total,
        calorías quemadas e intensidad promedio. Ideal para mejorar
        resistencia, potencia y coordinación.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaFistRaised size={28} color="#dc2626" />}>
        ...
        <MetricCard icon={<FaChartLine size={28} color="#10b981" />}>
          <p className="subtitulo">Promedio intensidad</p>
      ...
      <div className="graficas">
        ...
        <div className="chart-card">
          <p className="subtitulo">⏱️ Tiempo (min)</p>
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="minutos" stroke="#dc2626" dot={{ r: 3 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="chart-card">
          <p className="subtitulo">🔥 Calorías</p>
        ...
        <div className="chart-card">
          <p className="subtitulo">📈 Intensidad</p>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={data}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="fechaLabel" />
...
export default RegistroBoxeoSaco2;

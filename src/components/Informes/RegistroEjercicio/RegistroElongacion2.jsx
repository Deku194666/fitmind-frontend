...
import { FaChild, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroElongacion2.css";
...
function RegistroElongacion2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/elongacion/historial/${usuario_id}`,
        ...
        console.error("Error al cargar elongación:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "elongacion") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo">🧘‍♂️ Análisis de Elongación</p>

      <div className="intro">
        Revisa tus sesiones de elongación: tiempo dedicado, calorías quemadas
        e intensidad promedio. Ideal para mejorar flexibilidad y recuperación muscular.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaChild size={28} color="#8b5cf6" />}>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="minutos" stroke="#8b5cf6" dot={{ r: 3 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
...
export default RegistroElongacion2;

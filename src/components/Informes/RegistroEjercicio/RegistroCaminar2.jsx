...
import { FaWalking, FaClock, FaFire, FaChartLine, FaStar } from "react-icons/fa";
import "./RegistroCaminar2.css";
...
function RegistroCaminar2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/caminar/historial/${usuario_id}`,
        ...
        console.error("Error al cargar caminar:", err);
        if (!cancel) setData([]);
      } finally {
    ...
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "caminar") fetchRegistros();
    };
  ...
  return (
    <div className="registro-container">
      <p className="titulo">🚶‍♂️ Análisis de Caminatas</p>

      <div className="intro">
        Aquí puedes analizar tus sesiones de caminata: duración total,
        calorías quemadas e intensidad promedio. Observa tu progreso
        y mantén constancia en tu actividad diaria.
      </div>

      <div className="resumen">
        <MetricCard icon={<FaWalking size={28} color="#4f46e5" />}>
...
export default RegistroCaminar2;

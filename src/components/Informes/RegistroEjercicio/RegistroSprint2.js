...
function RegistroSprint2() {
  ...
  useEffect(() => {
    ...
    const fetchRegistros = async () => {
      try {
        ...
        // ⛳️ Requiere endpoint backend: GET /api/sprint/historial/:usuario_id
        const res = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/sprint/historial/${usuario_id}`,
    ...
    // 🔁 Auto-refresco al registrar desde el módulo de sprint
    const onNuevo = (e) => {
      if (!e?.detail?.tipo || e.detail.tipo === "sprint") fetchRegistros();
    };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <p className="Par1">📊 Análisis de Sprint</p>
      <p className="Par2">Visualiza la evolución de tus sesiones de sprint por intervalos.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Tiempo (min) */}
        <Card>
          <div><p className="ParGrafico">⏱️ Tiempo de sprint (min)</p></div>
...
export default RegistroSprint2;
